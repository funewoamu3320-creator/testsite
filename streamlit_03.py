...
st.header("3. ラジオボタン")
option = st.radio("検索条件を選択してください", ["条件1", "条件2", "条件3"])
st.write(f"{option}が選択されました")

st.header("4. セレクトボックス")
option = st.selectbox(
    "検索条件を選択してください", ["条件1", "条件2", "条件3", "条件4", "条件5"]
)
st.write(f"{option}が選択されました")
...
st.header("6. スライダー")
value = st.slider(label="値を選択してください", min_value=0, max_value=100, value=30)
...
st.header("8. テキストエリア")
text = st.text_area("検索するテキストを（複数行）入力してください")
st.write(f"{text}と入力されました")
